 ...
 * @category   library
 * @version    2.0.2 <beta> <Modified By ZettaSys>  
...
class Datatables
{
	 ...
	 * @return mixed
	 */
	public function having($key_condition, $val = NULL)
	{
		$this->having[] = array($key_condition, $val);
		$this->ci->db->having($key_condition, $val);
		return $this;
	}
	 ...
	 * @return mixed
	 */
	public function add_column($column, $content, $match_replacement = NULL, $search = FALSE)
	{
		$this->add_columns[$column] = array('content' => $content, 'replacement' => $this->explode(',', $match_replacement), 'search' => $search);
		return $this;
	}
		...
		if ($sSearch != '')
		{
			for ($i = 0; $i < count($mColArray); $i++)
			{
				if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
					$sWhere_or[] = $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' ESCAPE '!'";
			}
		}

		for ($i = 0; $i < count($mColArray); $i++)
		{
			if ($mColArray[$i]['searchable'] == 'true' && (!array_key_exists($mColArray[$i]['data'], $this->add_columns) || !empty($this->add_columns[$mColArray[$i]['data']]['search'])) && $mColArray[$i]['search']['value'] != '')
			{
				$sSearch = $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value']));
				if ($mColArray[$i]['search']['regex'] === 'exact') //Invento para usar select en filtros
				{
					if($sSearch === 'NULL')
					{
						$sWhere_and[] = $this->select[$mColArray[$i]['data']] . " IS NULL";
					} else {
