...
class Usuarios_oficinas extends MY_Controller
{
    ...
    public function listar()
    {
        ...
        $tableData = array(
            ...
            'source_url' => 'transferencias/usuarios_oficinas/listar_data',
            'reuse_var' => TRUE,
        ...
        $this->load_template('transferencias/usuarios_oficinas/usuarios_oficinas_listar', $data);
    }
    ...
    public function listar_data()
    {
        ...
        $this->datatables
                ->select('tr_usuarios_oficinas.id, users.username, personas.apellido as apellido, personas.nombre as nombre, tr_oficinas.nombre as oficina')
                ->from('tr_usuarios_oficinas')
                ->join('users', 'users.id = tr_usuarios_oficinas.user_id', 'left')
                ->join('personas', 'personas.id = users.persona_id', 'left')
                ->join('tr_oficinas', 'tr_oficinas.id = tr_usuarios_oficinas.oficina_id', 'left')
                ->add_column('ver', '<a href="transferencias/usuarios_oficinas/ver/$1" title="Ver" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>', 'id')
                ->add_column('editar', '<a href="transferencias/usuarios_oficinas/editar/$1" title="Editar" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>', 'id')
                ->add_column('eliminar', '<a href="transferencias/usuarios_oficinas/eliminar/$1" title="Eliminar" class="btn btn-primary btn-xs"><i class="fa fa-times"></i></a>', 'id');
    ...
    public function agregar()
    {
        ...
        if (in_groups($this->grupos_solo_consulta, $this->grupos))
        {
            ...
            redirect('transferencias/usuarios_oficinas/listar', 'refresh');
        }
        ...
        $this->array_user_control = $array_user = $this->get_array('Usuarios', 'usuario', 'id', array(
            ...
            'where' => array(
                ...
                array('column' => 'users.id NOT IN', 'value' => '(SELECT user_id FROM tr_usuarios_oficinas)', 'override' => TRUE)
            ),
            'sort_by' => 'personas.apellido, personas.nombre, username'
                )
        );
        $this->array_oficina_control = $array_oficina = $this->get_array('Oficinas', 'nombre', 'id', array(
            'where' => array(array('column' => 'nombre <>', 'value' => 'Escribano')),
            'sort_by' => 'nombre')
        );
        ...
        if ($this->form_validation->run() === TRUE)
        {
            $this->db->trans_begin();
            $trans_ok = TRUE;
            $trans_ok &= $this->Usuarios_oficinas_model->create(array(
                'user_id' => $this->input->post('user'),
                'oficina_id' => $this->input->post('oficina')), FALSE);
        ...
        $this->load_template('transferencias/usuarios_oficinas/usuarios_oficinas_abm', $data);
    }
    ...
    public function editar($id = NULL)
    {
        ...
        {
            $this->session->set_flashdata('error', 'Usuario sin permisos de edición');
            redirect("transferencias/usuarios_oficinas/ver/$id", 'refresh');
        }
        ...
        if (isset($_POST) && !empty($_POST))
        {
            ...
            if ($this->form_validation->run() === TRUE)
            {
                $this->db->trans_begin();
                $trans_ok = TRUE;
                $trans_ok &= $this->Usuarios_oficinas_model->update(array(
