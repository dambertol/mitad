...
class Usuarios_areas_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->table_name = 'in_usuarios_areas';
		$this->full_log = TRUE;
		...
		$this->default_join = array(
				array('users', 'users.id = in_usuarios_areas.user_id', 'LEFT'),
				array('personas', 'personas.id = users.persona_id', 'LEFT', array("CONCAT(personas.apellido, ', ', personas.nombre, ' (', username, ')') as user")),
				array('areas', 'areas.id = in_usuarios_areas.area_id', 'LEFT', array("CONCAT(areas.codigo, ' - ', areas.nombre) as area"))
		);
