...
class Usuarios_areas extends MY_Controller
{
    ...
    public function listar()
    {
        ...
        $tableData = array(
            ...
            'source_url' => 'vales_combustible/usuarios_areas/listar_data',
            'reuse_var' => TRUE,
        ...
        $this->load_template('vales_combustible/usuarios_areas/usuarios_areas_listar', $data);
    }
    ...
    public function listar_data()
    {
        ...
        $this->datatables
                ->select('vc_usuarios_areas.id, users.username, personas.apellido as apellido, personas.nombre as nombre, CONCAT(areas.codigo, \' - \', areas.nombre) as area')
                ->from('vc_usuarios_areas')
                ->join('users', 'users.id = vc_usuarios_areas.user_id', 'left')
                ->join('personas', 'personas.id = users.persona_id', 'left')
                ->join('areas', 'areas.id = vc_usuarios_areas.area_id', 'left')
                ->add_column('ver', '<a href="vales_combustible/usuarios_areas/ver/$1" title="Ver" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>', 'id')
                ->add_column('editar', '<a href="vales_combustible/usuarios_areas/editar/$1" title="Editar" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>', 'id')
                ->add_column('eliminar', '<a href="vales_combustible/usuarios_areas/eliminar/$1" title="Eliminar" class="btn btn-primary btn-xs"><i class="fa fa-times"></i></a>', 'id');
    ...
    public function agregar()
    {
        ...
        if (in_groups($this->grupos_solo_consulta, $this->grupos))
        {
            ...
            redirect('vales_combustible/usuarios_areas/listar', 'refresh');
        }
        ...
        $this->array_user_control = $array_user = $this->get_array('Usuarios', 'usuario', 'id', array(
            ...
            'where' => array(array('column' => 'groups.name', 'value' => 'vales_combustible_areas')),
        ...
        $this->load_template('vales_combustible/usuarios_areas/usuarios_areas_abm', $data);
    }
    ...
    public function editar($id = NULL)
    {
        ...
        {
            $this->session->set_flashdata('error', 'Usuario sin permisos de edición');
            redirect("vales_combustible/usuarios_areas/ver/$id", 'refresh');
        }
