...
class Usuarios_areas extends MY_Controller
{
	...
	public function listar()
	{
		...
		$this->load_template('notificaciones/usuarios_areas/usuarios_areas_listar', $data);
	}
	...
	public function listar_data()
	{
		...
		$this->datatables
				->select('nv_usuarios_areas.id, users.username, personas.apellido as apellido, personas.nombre as nombre, CONCAT(areas.codigo, \' - \', areas.nombre) as area')
				->from('nv_usuarios_areas')
				->join('users', 'users.id = nv_usuarios_areas.user_id', 'left')
				->join('personas', 'personas.id = users.persona_id', 'left')
				->join('areas', 'areas.id = nv_usuarios_areas.area_id', 'left')
				->add_column('ver', '<a href="notificaciones/usuarios_areas/ver/$1" title="Ver" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>', 'id')
				->add_column('editar', '<a href="notificaciones/usuarios_areas/editar/$1" title="Editar" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>', 'id')
				->add_column('eliminar', '<a href="notificaciones/usuarios_areas/eliminar/$1" title="Eliminar" class="btn btn-primary btn-xs"><i class="fa fa-times"></i></a>', 'id');
	...
	public function agregar()
	{
		...
		if (in_groups($this->grupos_solo_consulta, $this->grupos))
		{
			...
			redirect('notificaciones/usuarios_areas/listar', 'refresh');
		}
		...
		$this->array_user_control = $array_user = $this->get_array('Usuarios', 'usuario', 'id', array(
				...
				'where' => array(array('column' => 'groups.name', 'value' => 'notificaciones_areas')),
		...
		$this->load_template('notificaciones/usuarios_areas/usuarios_areas_abm', $data);
	}
	...
	public function editar($id = NULL)
	{
		...
		{
			$this->session->set_flashdata('error', 'Usuario sin permisos de edición');
			redirect("notificaciones/usuarios_areas/ver/$id", 'refresh');
		}
