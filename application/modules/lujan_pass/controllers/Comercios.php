...
class Comercios extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->grupos_admin = array('admin', 'lujan_pass_control', 'lujan_pass_consulta_general');
        $this->grupos_control = array('admin', 'lujan_pass_control');
        $this->grupos_permitidos = array('admin', 'lujan_pass_control', 'lujan_pass_publico', 'lujan_pass_consulta_general');
    ...
    public function listar()
    {
        ...
        $tableData = array(
            ...
            'source_url' => 'lujan_pass/comercios/listar_data',
            'reuse_var' => TRUE,
        ...
        $this->load_template('lujan_pass/comercios/comercios_listar', $data);
    }
    ...
    public function listar_data()
    {
        ...
        $this->load->helper('lujan_pass/datatables_functions_helper');
        $dt = $this->datatables
                ...
                ->where("ta_comercios.id IN (SELECT ta_comercios.id  
                    ...
                    WHERE ta_categorias.agrupamiento_id = $this->agrupamiento_id_turismo)");
        ...
        $dt->edit_column('estado', '$1', 'dt_column_comercios_estado(estado)', TRUE)
                ->add_column('ver', '<a href="lujan_pass/comercios/ver/$1" title="Ver" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>', 'id')
                ->add_column('editar', '<a href="lujan_pass/comercios/editar/$1" title="Editar" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>', 'id')
                ->add_column('anular', '<a href="lujan_pass/comercios/anular/$1" title="Anular" class="btn btn-primary btn-xs"><i class="fa fa-ban"></i></a>', 'id')
                ->add_column('eliminar', '<a href="lujan_pass/comercios/eliminar/$1" title="Eliminar" class="btn btn-primary btn-xs"><i class="fa fa-times"></i></a>', 'id');
    ...
    public function agregar()
    {
        ...
        if (in_groups($this->grupos_solo_consulta, $this->grupos))
        {
            ...
            redirect('lujan_pass/comercios/listar', 'refresh');
        }

        if (in_groups($this->grupos_admin, $this->grupos))
        {
            redirect('lujan_pass/comercios/agregar_admin', 'refresh');
        }
        ...
        $fake_model_comercio->fields = array(
            ...
            'latitud' => array('label' => 'Latitud', 'maxlength' => '20', 'type' => 'hidden', 'readonly' => TRUE),
            'longitud' => array('label' => 'Longitud', 'maxlength' => '20', 'type' => 'hidden', 'readonly' => TRUE),
        ...
        $this->array_categoria_control = $array_categoria = $this->get_array('Categorias', 'nombre', 'id', array('agrupamiento_id' => $this->agrupamiento_id_turismo));
        ...
        if ($this->form_validation->run() === TRUE)
        {
            ...
            if (!empty($_FILES['imagen_url']['name']))
            {
                ...
                $config['upload_path'] = "uploads/lujan_pass/comercios/";
        ...
        $lon = $this->form_validation->set_value('longitud', 'null');
        $data['lon'] = !empty($lon) ? $lon : 'null';
        $lat = $this->form_validation->set_value('latitud', 'null');
        $data['lat'] = !empty($lat) ? $lat : 'null';
        $data['nav'] = TRUE;
        ...
        $data['css'][] = 'vendor/openlayers/ol.css';
        ...
        $data['js'][] = 'vendor/openlayers/ol.js';
        $data['js'][] = 'js/lujan_pass/map.js';
        $this->load_template('lujan_pass/comercios/comercios_abm', $data);
    }
    ...
    public function agregar_admin()
    {
        ...
        $this->array_encargado_control = $array_encargado = $this->get_array('Personas', 'encargado', 'id', array(
            ...
            'where' => array(
                array('column' => 'groups.name IN', 'value' => "('lujan_pass_control', 'lujan_pass_publico')", 'override' => TRUE),
    ...
    public function editar($id = NULL)
    {
        ...
        {
            $this->session->set_flashdata('error', '<br />Usuario sin permisos de edición');
            redirect("lujan_pass/comercios/ver/$id", 'refresh');
        }
        ...
        $comercios_categorias = $this->Comercios_categorias_model->get(array(
            ...
            'where' => array(
                array('column' => 'ta_categorias.agrupamiento_id', 'value' => $this->agrupamiento_id_turismo)
            )
        ));
        ...
        $lon = $this->form_validation->set_value('longitud', !empty($comercio->longitud) ? $comercio->longitud : 'null');
        $data['lon'] = !empty($lon) ? $lon : 'null';
        $lat = $this->form_validation->set_value('latitud', !empty($comercio->latitud) ? $comercio->latitud : 'null');
        $data['lat'] = !empty($lat) ? $lat : 'null';
        $data['nav'] = TRUE;
    ...
    public function eliminar($id = NULL)
    {
        ...
        $data['lon'] = (!empty($comercio->longitud) ? $comercio->longitud : 'null');
        $data['lat'] = (!empty($comercio->latitud) ? $comercio->latitud : 'null');
        $data['nav'] = FALSE;
