...
class Datos_extra_oficinas_model extends MY_Model
{

	/**
	 * Modelo de Dato Extra Oficinas
	 * Autor: Leandro
	 * Creado: 12/08/2019
	...
	{
		parent::__construct();
		$this->table_name = 'rh_datos_extra_oficinas';
		$this->full_log = TRUE;
		$this->msg_name = 'Dato Extra Oficina';
		$this->id_name = 'id';
		$this->columnas = array('id', 'dato_extra_id', 'area_id');
		$this->fields = array();
		$this->requeridos = array('dato_extra_id', 'area_id');
		$this->unicos = array(array('dato_extra_id', 'area_id'));
	...
	/**
	 * intersect_asignaciones: Actualiza asignaciones de oficinas.
	 *
	...
	public function intersect_asignaciones($dato_extra_id, $new_asignaciones, $trans_enabled = false)
	{
		...
		if (!empty($old_asignaciones))
		{
			...
			{
				$old_asignaciones_array[$Old->id] = $Old->area_id;
			}
