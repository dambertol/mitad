...
class Inicio extends MY_Controller
{
    ...
    public function index($agrupamiento_id = NULL, $categoria_id = NULL)
    {
        ...
        {
            $agrupamiento_id = $this->agrupamiento_id_comercio;
        }

        $agrupamientos = $this->Agrupamientos_model->get(array('id' => $this->agrupamiento_id_comercio));
        ...
        if (empty($categorias))
        {
            redirect('mas_beneficios/front/inicio');
        }
        ...
        $data['agrupamiento_id_comercio'] = $this->agrupamiento_id_comercio;
        $data['categorias'] = $categorias;
        $data['image'] = '';
        $data['title'] = TITLE;
        $data['usuario_logueado'] = $this->ion_auth->logged_in();
        $this->load_template('mas_beneficios/front/inicio/inicio_content', $data);
    }
    ...
    public function get_comercios()
    {
        ...
        if ($this->form_validation->run() === TRUE)
        {
            $options = array(
                'select' => array(
                    'ta_comercios.id', 'ta_comercios.imagen_url', 'ta_comercios.nombre as comercio', "COALESCE(ta_comercios.comentarios, ' ') as comentarios", 'MIN(ta_categorias.nombre) as categoria', 'ta_comercios.web', 'ta_comercios.facebook', 'ta_comercios.instagram', 'ta_comercios.twitter'
                ),
                'join' => array(
            ...
            if (!empty($comercios))
            {
                foreach ($comercios as $key => $Comercio)
                {
                    ...
                    {
                        $comercios[$key]->imagen_url = 'img/mas_beneficios/noimage.png';
                    }
                }
    ...
    public function get_promociones()
    {
        ...
        if ($this->form_validation->run() === TRUE)
        {
            $options = array(
                'select' => array(
                    'ta_promociones.id', 'ta_promociones.imagen_url', 'ta_comercios.imagen_url as imagen_url_comercio', 'ta_comercios.nombre as comercio', "COALESCE(ta_promociones.descripcion, ' ') as comentarios", 'ta_campanias.nombre as categoria', 'ta_comercios.web', 'ta_comercios.facebook', 'ta_comercios.instagram', 'ta_comercios.twitter'
                ),
                'join' => array(
            ...
            if (!empty($promociones))
            {
                foreach ($promociones as $key => $Promocion)
                {
                    if (!file_exists($Promocion->imagen_url))
                    {
                        ...
                        {
                            $promociones[$key]->imagen_url = 'img/mas_beneficios/noimage.png';
                        }
                        else
                        {
            ...
            {
                $data['error'] = 'Promociones no encontradas';
            }
        }
        else
        {
    ...
    public function get_comercio()
    {
        ...
        $this->load->view('mas_beneficios/front/inicio/comercio_content', $data);
    }
    ...
    public function get_promocion()
    {
        ...
        if ($this->form_validation->run() === TRUE)
        {
            ...
            {
                $data['error'] = 'Promoción no encontrada';
            }
        }
        else
        {
        ...
        $this->load->view('mas_beneficios/front/inicio/promocion_content', $data);
    }
