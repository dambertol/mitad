...
class Adjuntos_tipos extends MY_Controller
{
    ...
    public function listar()
    {
        ...
        $tableData = array(
            ...
            'source_url' => 'tramites_online/adjuntos_tipos/listar_data',
            'reuse_var' => TRUE,
        ...
        $this->load_template('tramites_online/adjuntos_tipos/adjuntos_tipos_listar', $data);
    }
    ...
    public function listar_data()
    {
        ...
        $this->datatables
                ->select('id, nombre')
                ->from('to2_adjuntos_tipos')
                ->add_column('ver', '<a href="tramites_online/adjuntos_tipos/ver/$1" title="Ver" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>', 'id')
                ->add_column('editar', '<a href="tramites_online/adjuntos_tipos/editar/$1" title="Editar" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>', 'id')
                ->add_column('eliminar', '<a href="tramites_online/adjuntos_tipos/eliminar/$1" title="Eliminar" class="btn btn-primary btn-xs"><i class="fa fa-times"></i></a>', 'id');
    ...
    public function agregar()
    {
        ...
        if (in_groups($this->grupos_solo_consulta, $this->grupos))
        {
            ...
            redirect('tramites_online/adjuntos_tipos/listar', 'refresh');
        }
        ...
        if ($this->form_validation->run() === TRUE)
        {
            $this->db->trans_begin();
            $trans_ok = TRUE;
            $trans_ok &= $this->Adjuntos_tipos_model->create(
                    array(
        ...
        $this->load_template('tramites_online/adjuntos_tipos/adjuntos_tipos_abm', $data);
    }
    ...
    public function editar($id = NULL)
    {
        ...
        {
            $this->session->set_flashdata('error', '<br />Usuario sin permisos de edición');
            redirect("tramites_online/adjuntos_tipos/ver/$id", 'refresh');
        }
        ...
        if (isset($_POST) && !empty($_POST))
        {
            ...
            if ($this->form_validation->run() === TRUE)
            {
                $this->db->trans_begin();
                $trans_ok = TRUE;
                $trans_ok &= $this->Adjuntos_tipos_model->update(
                        array(
