...
class Ion_auth_model extends CI_Model
{
    ...
    public function activate($id, $code = FALSE)
    {
        ...
        if ($code !== FALSE)
        {
            ...
            if ($this->_full_log)
            {
                $this->db->query("INSERT INTO {$this->config->item('aud_db')}.{$this->tables['users']} SELECT NULL as audi_id, {$this->tables['users']}.* FROM {$this->tables['users']} WHERE id = $id");
                $data['audi_usuario'] = $this->session->userdata('user_id');
                $data['audi_fecha'] = date_format(new DateTime(), 'Y/m/d H:i:s');
                $data['audi_accion'] = 'U';
            }
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array())
    {
        ...
        $data = array(
            ...
            'username' => $identity,
            'password' => $password,
//		    'email'      => $email,	// Usando entidad Persona
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $tabla_users = $this->tables['users'];
        $query = $this->db->select($this->identity_column . ", personas.email, $tabla_users.id, persona_id, password, active, last_login, personas.nombre, personas.apellido, password_change")
                ->where($this->identity_column, $identity)
                ->join('personas', "personas.id = $tabla_users.persona_id")
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($tabla_users);
     ...
     * @author    MLC
     */
    public function login_auth0($identity, $email)
    {
        $this->trigger_events('pre_login');
    ...
    public function add_to_group($group_ids, $user_id = FALSE)
    {
        ...
        foreach ($group_ids as $group_id)
        {
            if ($this->_full_log)
            {
                $insert_ok = $this->db->insert($this->tables['users_groups'], array($this->join['groups'] => (float) $group_id, $this->join['users'] => (float) $user_id, 'audi_usuario' => $this->session->userdata('user_id'), 'audi_fecha' => date_format(new DateTime(), 'Y/m/d H:i:s'), 'audi_accion' => 'I'));
            }
            else
            {
                $insert_ok = $this->db->insert($this->tables['users_groups'], array($this->join['groups'] => (float) $group_id, $this->join['users'] => (float) $user_id));
            }
            if ($insert_ok)
            {
    ...
    public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
    {
        ...
        if (!empty($group_ids))
        {
            ...
            foreach ($group_ids as $group_id)
            {
                if ($this->_full_log)
                {
                    $this->db->query("INSERT INTO {$this->config->item('aud_db')}.{$this->tables['users_groups']} SELECT NULL as audi_id, {$this->tables['users_groups']}.* FROM {$this->tables['users_groups']} WHERE {$this->join['groups']}=" . (float) $group_id . " AND {$this->join['users']}=" . (float) $user_id);
                    ...
                    $this->db->where($this->join['groups'], (float) $group_id);
                    $this->db->where($this->join['users'], (float) $user_id);
                    $this->db->update($this->tables['users_groups']);
                    $this->db->query("INSERT INTO {$this->config->item('aud_db')}.{$this->tables['users_groups']} SELECT NULL as audi_id, {$this->tables['users_groups']}.* FROM {$this->tables['users_groups']} WHERE {$this->join['groups']}=" . (float) $group_id . " AND {$this->join['users']}=" . (float) $user_id);
                }
    ...
    public function update($id, array $data)
    {
        ...
        if ($this->_full_log)
        {
            $this->db->query("INSERT INTO {$this->config->item('aud_db')}.{$this->tables['users']} SELECT NULL as audi_id, {$this->tables['users']}.* FROM {$this->tables['users']} WHERE id = $user->id");
            $data['audi_usuario'] = $this->session->userdata('user_id');
            $data['audi_fecha'] = date_format(new DateTime(), 'Y/m/d H:i:s');
            $data['audi_accion'] = 'U';
        }
    ...
    public function login_remembered_user()
    {
        ...
        $this->trigger_events('extra_where');
        $tabla_users = $this->tables['users'];
        $query = $this->db->select($this->identity_column . ", $tabla_users.id, persona_id, personas.email, last_login, personas.nombre, personas.apellido, password_change")
                ->join('personas', "personas.id = $tabla_users.persona_id")
